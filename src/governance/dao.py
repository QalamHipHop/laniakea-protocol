...
class GovernanceSystem:
    ...
    def create_proposal(
        ...
        Returns:
            پیشنهاد ایجاد شده
        """
        proposal_id = hashlib.sha256(f"{proposer_id}{title}{time()}".encode()).hexdigest()
    ...
    def cast_vote(
        self, voter_id: str, proposal_id: str, vote: bool, reason: Optional[str] = None
    ) -> bool:
        """
        ثبت رأی
    ...
    def get_proposal_details(self, proposal_id: str) -> Optional[Dict]:
        ...
        return {
            ...
            "participation": (
                (proposal.votes_for + proposal.votes_against) / sum(self.voter_stakes.values())
                if self.voter_stakes
                else 0
            ),
        }
...
class AutoGovernance:
    ...
    def analyze_proposal(self, proposal_id: str) -> Dict:
        ...
        return {"proposal_id": proposal_id, "message": "AI analysis not available"}
