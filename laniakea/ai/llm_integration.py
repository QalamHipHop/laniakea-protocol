...
class PollinationsProvider:
    """Pollinations.ai — **no API key required** real LLM inference.

    Pollinations (https://text.pollinations.ai) is a free, public inference
    gateway that proxies to several real production models (openai-fast,
    openai, mistral, llama, qwen-coder, …). We use it as a *zero-config*
    fallback that still produces real, model-generated text — never a
    canned response.

    Endpoint pattern: ``GET https://text.pollinations.ai/<urlencoded prompt>``
    Query params:
      * ``model``    — e.g. ``openai-fast``, ``openai``, ``llama``, ``mistral``
      * ``seed``     — int for reproducibility
      * ``json``     — ``true`` to force JSON output
      * ``temperature`` / ``top_p`` etc.
    """

    name = "pollinations"
    DEFAULT_MODEL = os.getenv("POLLINATIONS_MODEL", "openai-fast")
    BASE_URL = os.getenv("POLLINATIONS_BASE", "https://text.pollinations.ai")
    ...
    def complete(
        ...
        *,
        ...
        # Pollinations expects the full instruction in the URL path. We
        # concatenate system + user so the model sees the system prompt.
        full_prompt = (
        ...
        params = {
            ...
            "seed": str(random.randint(0, 2_000_000_000)),
            "temperature": str(temperature),
        }
        if json_mode:
            params["json"] = "true"
        q = urllib.parse.urlencode(params)
        url = f"{self.base_url}/{encoded}?{q}"
        req = urllib.request.Request(url, headers={"User-Agent": "laniakea-protocol/6.0"})
        # Fail fast: a 25s ceiling means a single provider outage can
        # never block the API for more than ~25s × 3 retries.
        try:
            with urllib.request.urlopen(req, timeout=25) as resp:  # nosec - public endpoint
                data = resp.read().decode("utf-8", errors="replace").strip()
                if not data:
                    raise RuntimeError("pollinations: empty response")
                if json_mode and not data.startswith("{"):
                    # Try to recover by slicing from first '{' to last '}'
                    i, j = data.find("{"), data.rfind("}")
                    if i >= 0 and j > i:
                        data = data[i : j + 1]
                return data
        except urllib.error.HTTPError as exc:
            raise RuntimeError(f"pollinations HTTP {exc.code}: {exc.reason}") from exc
        except urllib.error.URLError as exc:
            raise RuntimeError(f"pollinations network error: {exc.reason}") from exc


class OpenRouterProvider:
    """OpenRouter.ai — unified gateway to 100+ production models.

    OpenRouter exposes an OpenAI-compatible Chat Completions API. Free
    models (suffix ``:free``) are available without a credit card, e.g.
    ``meta-llama/llama-3.3-70b-instruct:free``, ``qwen/qwen-2.5-coder-32b-
    instruct:free``, ``google/gemma-2-9b-it:free`` and many more.

    Activated when ``OPENROUTER_API_KEY`` is set in the environment. We
    do NOT call this without a key — anonymous traffic to OpenRouter is
    rate-limited to ~20 req/day and is not safe for production.
    """

    name = "openrouter"
    DEFAULT_MODEL = os.getenv("OPENROUTER_MODEL", "meta-llama/llama-3.3-70b-instruct:free")
    BASE_URL = os.getenv("OPENROUTER_BASE", "https://openrouter.ai/api")
    ...
    def _headers(self) -> Dict[str, str]:
        ...
        return {
            ...
            # OpenRouter recommends advertising the app for rankings.
            "HTTP-Referer": os.getenv("OPENROUTER_REFERRER", "https://laniakea-protocol.onrender.com"),
            "X-Title": "LaniakeA Protocol",
        }
    ...
    def complete(
        ...
        *,
        ...
        if json_mode:
            body["response_format"] = {"type": "json_object"}
        resp = _http_post_json(url, self._headers(), body, timeout=60.0)
        try:
            return resp["choices"][0]["message"]["content"]
        except (KeyError, IndexError, TypeError) as exc:
            raise RuntimeError(f"openrouter: unexpected response: {resp}") from exc
...
class GroqProvider:
    """Groq Cloud — OpenAI-compatible, very fast inference, generous free tier.

    Activated when ``GROQ_API_KEY`` is set in the environment. Default model
    is ``llama-3.1-8b-instant`` which is free with high rate limits.
    See https://console.groq.com/docs/rate-limits for current limits.
    """

    name = "groq"
    DEFAULT_MODEL = os.getenv("GROQ_MODEL", "llama-3.1-8b-instant")
    BASE_URL = os.getenv("GROQ_BASE", "https://api.groq.com/openai")
    ...
    def complete(
        ...
        *,
        ...
        if json_mode:
            body["response_format"] = {"type": "json_object"}
        resp = _http_post_json(url, self._headers(), body, timeout=30.0)
        try:
            return resp["choices"][0]["message"]["content"]
        except (KeyError, IndexError, TypeError) as exc:
            raise RuntimeError(f"groq: unexpected response: {resp}") from exc
        ...
        for name in order:
            ...
            for attempt in range(self.policy.max_retries + 1):
                try:
                    ...
                    # Don't waste retries on hard-billing / auth errors
                    is_fatal = any(t in err_str for t in (
                        "402", "payment required", "401", "unauthorized",
                        "403", "forbidden", "404", "not found",
                    ))
                    logger.warning(
                        "LLM call failed (provider=%s attempt=%d%s): %s",
                        name, attempt,
                        " [fatal, skip retry]" if is_fatal else "",
                        exc,
                    )
                    if is_fatal:
                        break  # immediately try the next provider
...
def build_default_client() -> LLMClient:
    ...
    # Pollinations is ALWAYS registered (no key required) and becomes
    # the default unless a higher-priority provider (OpenAI/Anthropic/
    # OpenRouter) is also configured. This means the AI subsystem
    # produces **real** model output out of the box, even in a brand
    # new deployment with no secrets configured.
    providers.append(PollinationsProvider())
    if default_provider == "stub":
        default_provider = "pollinations"
        default_model = os.getenv("POLLINATIONS_MODEL", PollinationsProvider.DEFAULT_MODEL)

    if os.getenv("OPENROUTER_API_KEY"):
        providers.append(OpenRouterProvider())
        if default_provider in ("stub", "pollinations"):
            default_provider = "openrouter"
            default_model = os.getenv("OPENROUTER_MODEL", OpenRouterProvider.DEFAULT_MODEL)
    # Only trust a *real* OpenAI key (i.e. one that does not start with
    # the placeholder ``dummy-`` family). The committed ``.env`` ships
    # ``dummy-key-for-testing``; we still want real AI to run, so we
    # keep Pollinations as the active provider in that case.
    _openai_key = os.getenv("OPENAI_API_KEY", "")
    if _openai_key and not _openai_key.lower().startswith("dummy"):
    ...
    if os.getenv("GROQ_API_KEY"):
        ...
        # Groq wins because it's fast + free tier
        default_provider = "groq"
        default_model = os.getenv("GROQ_MODEL", GroqProvider.DEFAULT_MODEL)
