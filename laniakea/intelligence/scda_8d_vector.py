"""
LaniakeA Protocol - 8D Vector SCDA (numpy-based legacy implementation)
====================================================================

The historical ``SCDA`` class referenced by the README's block-equation
section (K_req . A = D(P) . E). It models the 8-dimensional state
vector S(t) = (K(t), E(t)) where K(t) is a 4D knowledge vector and E(t) is
a 4D energy vector.

This module is kept for backward compatibility with the white-paper and
for scientific simulations that benefit from the explicit numpy
representation. New code should use :class:`laniakea.intelligence.scda_model.SingleCellDigitalAccount`
which integrates Digital DNA, breeding, tier systems, and the 8D
knowledge-vector pipeline.
"""
...
class SCDA:
    ...
    Re-exported as :class:`SCDA8D` for canonical imports and as
    :class:`LaniakeaSCDA` for the white-paper-compatible name.
    """
...
# --- Backward-compatible aliases (used in tests, notebooks, white-paper) ----
SCDA8D = SCDA
LaniakeaSCDA = SCDA


def get_8d_state_vector(scda: SCDA) -> np.ndarray:
    """Return the full 8D state vector S(t) = (K(t), E(t))."""
    return scda.state_vector
