...
class LaniakeaSettings(BaseSettings):
    ...
    # v6.3.0-Qalam — additive upgrade over v6.2.0-Qalam.
    # Adds: Cosmic UI v8 (Qalam) — modern 8D glassmorphism dashboard
    # with 18+ real-time subsystems, full mobile-first responsive design,
    # 8-dimensional hypercube WebGL projection, live activity feed,
    # quick-actions for every API subsystem (SCDA, AI, Quantum, DeFi,
    # NFT, Knowledge Market, Diplomacy, Governance, Mining, Simulation).
    # All 154+ existing routes preserved 100% — purely additive.
    # v6.2.0-Qalam — additive upgrade over v6.0.1-Mainnet / v6.1.0-Qalam.
    # Adds: /v6/qalam/status, /v6/feed (live activity), /v6/scda/leaderboard,
    # /v6/cosmic/overview (richer payload), /v6/contract/{name} (Smart
    # Contract VM), Real-time Live Activity Feed in Cosmic v6 UI,
    # upgraded 8D Hypercube animation, mobile-380px breakpoint, dark/light
    # theme toggle, export endpoints, Qalam branding in About.
    # No existing endpoint behaviour is changed — every change is additive.
    PROJECT_VERSION: str = "6.3.0-Qalam"
    NODE_ID: str = "laniakea-node"
    # v6.0.0-Mainnet default: production. Individual deployments can still
    # override via DEPLOYMENT_ENV=development in their environment.
    DEPLOYMENT_ENV: str = "production"
    DEBUG: bool = False
    # Network selection (mainnet/testnet). Surfaced via / and /health
    # so dashboards can detect "real" deployments at a glance.
    NETWORK_MODE: str = "mainnet"
    NETWORK_ID: str = "laniakea-mainnet"
    CHAIN_ID: str = "1"
