    ...
    def execute_contract(self, 
        ...
        try:
            ...
            if function_name == "set_value":
                ...
                # Graceful fallback: treat unknown functions as a no-op echo.
                # This keeps the VM functional for arbitrary / experimental
                # contract calls and avoids crashing the API surface.
                gas_system.consume_gas(300)
                echo_payload = {
                    "function": function_name,
                    "args": args or {},
                    "note": "unknown function — echoed as no-op",
                }
                result = ExecutionResult(
                    True,
                    gas_system.get_gas_used(),
                    return_value=echo_payload,
                    logs=[f"Echoed unknown function {function_name}"],
                )
