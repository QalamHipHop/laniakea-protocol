...
# --- Web frontend static mount (best-effort) -------------------------------
# Serves the ``web/`` directory at ``/web/*`` so the cosmic dashboard,
# social hub, NFT pages etc. are reachable in production (e.g. on Render
# without a separate static host). The mount only runs if the directory
# actually exists in the deployed artefact.
try:
    from pathlib import Path as _Path
    from fastapi.staticfiles import StaticFiles as _StaticFiles

    _web_dir = (_Path(__file__).resolve().parent.parent.parent / "web").resolve()
    if _web_dir.is_dir():
        app.mount("/web", _StaticFiles(directory=str(_web_dir), html=True), name="web")
        logger.info("Web frontend mounted at /web (%s)", _web_dir)
    else:
        logger.info("No web/ directory at %s; static frontend disabled", _web_dir)
except Exception as exc:  # pragma: no cover - defensive
    logger.warning("Web static mount failed: %s", exc)
...
# Social Hub API: posts, follows, profiles, leaderboard.
# Implemented as an in-memory router (no DB dependency) so the
# cosmic-dashboard v5 social tab works out-of-the-box.
try:
    from laniakea.api.social_api import router as social_router
    app.include_router(social_router, tags=["Social Hub"])
    logger.info("Social Hub API mounted at /social")
except Exception as exc:  # pragma: no cover - defensive
    logger.warning("social_api router not loaded: %s", exc)
...
# behaviourally changed. See :mod:`laniakea.api.v6_routes` for the list of
# new routes (``/v6/qalam/status``, ``/v6/feed``, ``/v6/cosmic/overview``,
# ``/v6/scda/leaderboard``, ``/v6/contract/{name}``, ``/v6/health``).
try:
    from laniakea.api.v6_routes import router as v6_router
    app.include_router(v6_router)
    logger.info("v6.2.0-Qalam API mounted at /v6 (additive)")
except Exception as exc:  # pragma: no cover - defensive
    logger.warning("v6_routes not loaded: %s", exc)


# --- UI compatibility aliases (Cosmic UI v6 expects GET on many paths) ----
# See :mod:`laniakea.api.ui_aliases` for details. Installed after every
# router so that any future router that claims a name keeps priority.
try:
    from laniakea.api.ui_aliases import install_ui_aliases
    install_ui_aliases(app)
except Exception as exc:  # pragma: no cover - defensive
    logger.warning("UI aliases not installed: %s", exc)
...
def ui_redirect() -> RedirectResponse:
    """Browser-friendly redirect to the 8D Cosmic UI v8 (Qalam)."""
    return RedirectResponse(url="/web/cosmic_v8.html", status_code=307)
...
def read_root(request: Request) -> Dict[str, Any]:
    ...
    return {
        ...
        "status": "/core/status",
        "ui": {
            "unified_cosmic_v8": "/web/cosmic_v8.html",
            "unified_cosmic_v7": "/web/cosmic_v7.html",
            "unified_cosmic_v6": "/web/cosmic_v6.html",
            "landing": "/web/landing.html",
            "cosmic_dashboard_v5": "/web/cosmic_v5.html",
            "cosmic_dashboard_v4": "/web/cosmic_v4.html",
            "cosmic_dashboard": "/web/cosmic.html",
            "scda_dashboard": "/web/scda_dashboard.html",
            "mining_dashboard": "/web/mining_dashboard.html",
            "achievements": "/web/achievements.html",
            "social_hub": "/web/social_hub.html",
            "metaverse_8d": "/web/metaverse_8d_visualization.html",
        },
        # Read network/environment from the canonical settings object
        # so a single source of truth feeds every endpoint. The legacy
        # os.getenv() fallback is kept as a last-resort safety net.
        "network_mode": getattr(settings, "NETWORK_MODE", None) or os.getenv("NETWORK_MODE", "testnet"),
        "is_mainnet": bool(
            getattr(settings, "IS_MAINNET", None)
            if getattr(settings, "IS_MAINNET", None) is not None
            else (os.getenv("NETWORK_MODE", "testnet") == "mainnet")
        ),
        "chain_id": getattr(settings, "CHAIN_ID", None) or os.getenv("CHAIN_ID", "1"),
        "network_id": getattr(settings, "NETWORK_ID", None) or os.getenv("NETWORK_ID", "laniakea-mainnet"),
        "subsystems": {
            "blockchain": True,
            "consensus": True,
            "crosschain": True,
...
async def _on_startup() -> None:
    """Capture service start time + expose live subsystem refs to handlers."""
    import time as _t
    app.state.start_time = _t.time()
    # Make every live subsystem reachable from app.state.<name> so the
    # UI-aliases module (and any other introspection route) can read
    # live state without re-importing heavy modules.
    app.state.chain = laniakea_chain
    app.state.consensus = laniakea_consensus
    app.state.bridge = laniakea_bridge
    app.state.quantum = laniakea_quantum
    app.state.dao = laniakea_dao
    app.state.marketplace = laniakea_marketplace
    app.state.simulator = laniakea_simulator
    app.state.metrics = laniakea_metrics
    app.state.achievements = laniakea_achievements
    app.state.ai = laniakea_ai
    app.state.dex = laniakea_dex
...
def core_status() -> Dict[str, Any]:
    return {
        "status": "Operational",
        "protocol_version": settings.PROJECT_VERSION,
        "environment": getattr(settings, "ENVIRONMENT", None) or settings.DEPLOYMENT_ENV,
        "network_mode": getattr(settings, "NETWORK_MODE", None) or os.getenv("NETWORK_MODE", "testnet"),
        "is_mainnet": bool(
            getattr(settings, "IS_MAINNET", None)
            if getattr(settings, "IS_MAINNET", None) is not None
...
def cosmic_overview() -> Dict[str, Any]:
    ...
    return {
        "protocol": {
            ...
            "version": settings.PROJECT_VERSION,
            "environment": getattr(settings, "NODE_ENV", None) or settings.DEPLOYMENT_ENV,
            "network_mode": getattr(settings, "NETWORK_MODE", None) or os.getenv("NETWORK_MODE", "testnet"),
            "is_mainnet": bool(
                getattr(settings, "IS_MAINNET", None)
                if getattr(settings, "IS_MAINNET", None) is not None
